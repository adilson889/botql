...
this.value = value;
this.line = line;
this.start = start; // índice do 1º caractere deste token no source
this.end = end;      // índice logo a seguir ao último caractere (exclusivo)
}
}

// Erro de sintaxe com posição exata no texto (start/end, índices de
// caractere) — para além da mensagem (inglês, inalterada). O editor usa
// start/end para realçar o trecho certo (ex: editor.markText com
// CodeMirror), sem precisar de números de linha nem de gutter.
class BotQLSyntaxError extends Error {
constructor(message, { line = null, start = 0, end = null, near = null } = {}) {
super(message);
this.name = 'BotQLSyntaxError';
this.line = line;
this.start = start;
...
error(msg, start, end) {
const s = start !== undefined ? start : this.pos;
throw new BotQLSyntaxError(`syntax error: ${msg}`, { line: this.line, start: s, end });
}
...
token = new Token(TokenType.SYMBOL, c, this.line);
this.pos++;
}
...
this.error(`unexpected character: "${c}"`, startPos, startPos + 1);
}
...
const eofToken = new Token(TokenType.EOF, null, this.line, eofPos, eofPos);
...
this.error('unterminated string', quoteStart, this.pos);
}
...
const near = tok && tok.type !== TokenType.EOF ? tok.value : 'EOF';
const start = tok ? tok.start : this.source.length;
const end = tok ? tok.end : this.source.length;
throw new BotQLSyntaxError(`syntax error: ${msg}, near "${near}"`, { line: tok ? tok.line : null, start, end, near });
}
...
if (this.atKeyword('WAITING')) return this.parseWaitingStatement();
...
if (this.atKeyword('SHOW')) return this.parseShow();
...
// ---- SHOW CATALOG(ficheiro.txt) [CATEGORY("...")] ----
// ---- SHOW CATEGORY(ficheiro.txt) ----
//
// CATALOG mostra os itens (nome/preço/cor/imagem) — de uma categoria só,
// se vier CATEGORY("..."), ou de todas juntas sem isso.
// CATEGORY sozinho (sem estar preso a um CATALOG) mostra só os NOMES das
// categorias do ficheiro, como botões — pensado para o utilizador
// escolher a categoria primeiro, e essa escolha (o nome tocado) volta
// como mensagem normal, batendo num CONTAINS mais à frente (ver botql.js,
// execStatement caso 'ShowCategory').
parseShow() {
this.expectKeyword('SHOW');

if (this.atKeyword('CATEGORY')) {
this.advance();
this.expect(TokenType.SYMBOL, '(');
const file = this.parseFileName();
this.expect(TokenType.SYMBOL, ')');
return { type: 'ShowCategory', file };
}
...
this.advance();
this.expect(TokenType.SYMBOL, '(');
category = this.advance().value; // STRING
this.expect(TokenType.SYMBOL, ')');
}

return { type: 'ShowCatalog', file, category };
}
...
// ---- WAITING() | WAITING("texto") | WAITING("texto", N) ----
// ---- WAITING(ficheiro.txt) | WAITING(ficheiro.txt, N) ----
//
// Duas formas de uso:
//   1. Acoplado a um THINK, logo a seguir a ele, antes do OR (ver
//      parseThink acima) — reaproveita o tempo da própria busca.
//   2. Sozinho, como ação normal dentro de um bloco — útil antes de
//      qualquer outra ação que também possa demorar (ex: Response(),
//      que depende de uma IA externa) e não tem nenhum aviso próprio.
// Aceita texto direto entre aspas (mais rápido para uma frase curta,
// sem precisar de criar um .txt só para isso) ou nome de ficheiro sem
// aspas (para HTML/animação mais elaborada). Os dois argumentos são
// opcionais e independentes: sem nenhum, o interpretador usa texto e
// tempo mínimo por omissão ("Pensando...", 3 segundos) — ver botql.js,
...
this.expect(TokenType.SYMBOL, ')');
return { file, text, seconds };
}

// WAITING(...) usado como ação solta no corpo de um bloco (não logo a
// seguir a um THINK) — mesma sintaxe de argumentos de tryParseWaiting,
// só muda o que acontece a seguir: aqui é sempre uma ação própria, com
// o resto do bloco a continuar normalmente depois dela.
parseWaitingStatement() {
const cfg = this.tryParseWaiting();
return { type: 'Waiting', file: cfg.file, text: cfg.text, seconds: cfg.seconds };
}
