...
// Ternlight (embeddings semânticos, WASM local) — usado só como
// conselheiro em cima da decisão do BM25 (ver analyze()), nunca decide
// o ranking sozinho. Se a pasta não existir, o THINK continua a
// funcionar normalmente, só sem essa rede de segurança extra para
// sinónimos sem raiz comum.
//
// Caminho relativo, não nome de pacote — RAG.js vive em assets/, ao
// lado de ternlight-mini/ (copiada manualmente, sem node_modules nem
// npm install nesse ambiente). index.js (não src/index.js) é o
// adaptador que usa pkg-web/tern_engine.js com initSync() via bytes
// lidos pela bridge Android — src/ é o código-fonte Rust/Node original,
// incompatível com o WebView (ver ternlight-mini/index.js para o porquê).
let ternlight = null;
let ternlightErroCarregamento = null;
try {
ternlight = require('./ternlight-mini/index.js');
} catch (e) {
ternlight = null;
ternlightErroCarregamento = e && e.message ? e.message : String(e);
}

// cosineSim do próprio pacote já assume vetores L2-normalizados (é só
// um dot product, sem sqrt) — mais rápido que uma cosseno genérica.
function cosseno(a, b) {
return ternlight.cosineSim(a, b);
}

// Limiar de similaridade de cosseno do Ternlight a partir do qual o
// conselho semântico é considerado forte o suficiente para override —
// calibrado com testes reais (par sinônimo claro: ~0.67; par
// irrelevante: ~0.33) deixando margem confortável acima do ruído.
const CONFIANCA_SEMANTICA_MINIMA = 0.5;

// ---- DEBUG TEMPORÁRIO ----
// Sem acesso fácil ao Logcat neste momento — esta função dá visibilidade
// do estado real do Ternlight (carregou? inicializou? que erro deu?)
// através do próprio chat, chamando-a manualmente. Remover depois de
// confirmado que o motor está operacional.
function diagnosticoTernlight() {
const linhas = [];
linhas.push('ternlight require(): ' + (ternlight ? 'OK' : 'FALHOU'));
if (ternlightErroCarregamento) {
linhas.push('erro no require: ' + ternlightErroCarregamento);
}
// O pré-carregamento (tern_engine.js / ternlight-mini/index.js) roda
// no index.html, ANTES do RAG.js sequer carregar -- se falhar ali, o
// erro real (ex: asset não encontrado) só ia parar no console.error
// do Logcat, nunca chegava aqui. index.html guarda esse erro em
// window.__botqlTernlightPreloadErro quando acontece.
if (typeof window !== 'undefined' && window.__botqlTernlightPreloadErro) {
linhas.push('erro no pré-carregamento (index.html): ' + window.__botqlTernlightPreloadErro);
}
if (ternlight) {
try {
const ok = ternlight.inicializar();
linhas.push('inicializar(): ' + (ok ? 'OK' : 'devolveu false'));
if (ok) {
const v = ternlight.embed('teste');
linhas.push('embed() devolveu vetor de tamanho ' + v.length);
}
} catch (e) {
linhas.push('erro ao inicializar/embed: ' + (e && e.message ? e.message : String(e)));
}
}
return linhas.join(' | ');
}
...
const PESO_BM25 = 1;
const PESO_FRASE = 4; // bónus agora é uma fração 0-1 (ver _bonusFrase), não soma sem limite
const PESO_FUZZY = 0.4;
const PESO_TITULO = 3;
const ZONA_TITULO = 8; // primeiras N palavras significativas do bloco
...
this.docPalavrasSignificativas = this.blocks.map((bloco) => this._palavrasSignificativas(bloco));

// Vetores semânticos: lazy, só calculados na primeira vez que o
// conselho semântico é mesmo consultado (ver _conselhoSemantico) —
// a maioria das mensagens é resolvida só pelo BM25 e nunca chega a
// precisar disto.
this._vetoresBlocos = null;
...
// Palavras "de conteúdo" de um texto: normalizadas, minúsculas, sem
// pontuação, SEM stopwords — mas sem stem, porque aqui o que importa é
// a sequência literal de palavras, não a raiz. Usado só pelo bónus de
// frase (_bonusFrase), não pelo BM25 (que usa tokenizar(), com stem).
_palavrasSignificativas(texto) {
return normalizar(texto)
.toLowerCase()
.replace(/[^a-z0-9\s]/g, ' ')
.split(/\s+/)
...
// Bónus de frase: procura a maior sequência contígua de palavras "de
// conteúdo" da pergunta que aparece, também contígua, no bloco — mas
// SÓ contando as palavras significativas dos dois lados, ignorando
// "de"/"da"/"do"/etc. Isso é o que faz "Reino Matamba" (sem artigo,
// como na lista geral) e "Reino da Matamba" (o texto real, com artigo)
// contarem como a MESMA frase — sem isto, o artigo sozinho decidia
// qual bloco ganhava, mesmo quando o outro era a resposta certa.
//
// Devolve uma fração entre 0 e 1 (quanto da pergunta ficou coberto
// pela maior frase encontrada), não um valor que cresce sem limite —
// evita que um match de frase comprido esmague sozinho o resto do
// score (ver PESO_FRASE em _scoreDoc).
_bonusFrase(message, docIndex) {
const palavrasQuery = this._palavrasSignificativas(message);
if (palavrasQuery.length < 2) return 0;

const palavrasBloco = this.docPalavrasSignificativas[docIndex];

let melhorTamanho = 0;
for (let tamanho = Math.min(6, palavrasQuery.length); tamanho >= 2; tamanho--) {
for (let i = 0; i + tamanho <= palavrasQuery.length; i++) {
const sub = palavrasQuery.slice(i, i + tamanho);
if (this._contemSequencia(palavrasBloco, sub)) {
melhorTamanho = tamanho;
break;
}
}
if (melhorTamanho > 0) break;
}

return melhorTamanho === 0 ? 0 : melhorTamanho / palavrasQuery.length;
}

_contemSequencia(arr, sub) {
...
// Bónus de "início do bloco": quantas palavras significativas da
// pergunta aparecem logo nas primeiras ZONA_TITULO palavras do bloco
// (onde normalmente está o assunto/título dele, ex: "Reino da Matamba:
// Consolidado por..."), em vez de espalhadas ou só no fim. Um bloco
// curto que só MENCIONA o assunto de passagem (ex: numa lista) tende a
// ter a palavra-chave mais para o fim, não logo no início — é isso que
// distingue "é sobre isto" de "só falou disto de passagem".
_bonusTitulo(palavrasQuery, docIndex) {
if (palavrasQuery.length === 0) return 0;
const zona = new Set(this.docPalavrasSignificativas[docIndex].slice(0, ZONA_TITULO));
const unicas = new Set(palavrasQuery);
let encontrados = 0;
for (const p of unicas) if (zona.has(p)) encontrados++;
return encontrados / unicas.size;
}
...
const titulo = this._bonusTitulo(this._palavrasSignificativas(message), docIndex);
return PESO_BM25 * bm25 + PESO_FRASE * frase + PESO_TITULO * titulo;
}
...
// Conselheiro semântico: só chamado quando o BM25 já ficou indeciso
// (UNKNOWN ou REANALISAR) — nunca decide o ranking geral sozinho.
// candidatos, se passado, restringe a comparação a só esses índices
// (ex: melhor/segundo do REANALISAR); sem candidatos, compara contra
// todos os blocos (caso UNKNOWN, sem nenhum candidato do BM25 pra
// restringir a busca).
//
// Devolve { index, score } do bloco mais parecido semanticamente, ou
// null se o Ternlight não estiver disponível ou nada bater com
// confiança suficiente (ver CONFIANCA_SEMANTICA_MINIMA).
_conselhoSemantico(message, candidatos) {
if (!ternlight) return null;

if (this._vetoresBlocos === null) {
this._vetoresBlocos = this.blocks.map((bloco) => ternlight.embed(bloco));
}

const indices = candidatos || this.blocks.map((_, i) => i);
const vetorMsg = ternlight.embed(message);

let melhorIndex = -1;
let melhorScore = -1;
for (const i of indices) {
const score = cosseno(vetorMsg, this._vetoresBlocos[i]);
...
if (melhorIndex === -1 || melhorScore < CONFIANCA_SEMANTICA_MINIMA) return null;
return { index: melhorIndex, score: melhorScore };
}
...
analyze(message) {
// ---- DEBUG TEMPORÁRIO ----
// Atalho para inspecionar o estado real do Ternlight através do
// próprio chat, sem precisar de Logcat. Remover esta condição depois
// de confirmado que o motor está operacional.
if (message.trim().toLowerCase() === 'diagnostico ternlight') {
return {
decision: 'RESPONDER', confidence: 1, margem: 1, texto: diagnosticoTernlight(),
unificado: false, reanalisado: false, melhor: null, segundo: null
};
}
...
// BM25 não achou nada em comum — antes de desistir, pergunta ao
// conselheiro semântico se algum bloco é parecido em SIGNIFICADO
// mesmo sem palavras em comum (ex: "presidente" vs "líder").
const conselho = this._conselhoSemantico(message, null);
if (conselho) {
const info = { text: this.blocks[conselho.index], score: conselho.score, index: conselho.index };
return {
decision: 'RESPONDER', confidence: conselho.score, margem: 1, texto: info.text,
unificado: false, reanalisado: false, melhor: info, segundo: null
};
}
return vazio;
}
...
// Léxico teve algum candidato, mas fraco demais pra confiar. Mesma
// lógica do caso acima: pergunta ao conselheiro semântico antes de
// desistir de vez.
const conselho = this._conselhoSemantico(message, null);
if (conselho) {
const info = { text: this.blocks[conselho.index], score: conselho.score, index: conselho.index };
...
// O BM25 decidiu com confiança — mas confiança lexical não é o
// mesmo que estar semanticamente certo. Palavras comuns e pouco
// decisivas ("is", "the", "trump") podem dar confiança/margem
// altas mesmo quando o bloco não responde ao que foi perguntado
// (ex.: "qual a nacionalidade" bater com "é o líder", só porque
// as duas frases repetem "Trump"/"USA"). Por isso, mesmo aqui,
// pergunta ao conselheiro semântico se um OUTRO bloco é muito
// mais parecido em significado — e só troca a decisão nesse caso
// (discordância forte), nunca por preferência leve.
const conselho = this._conselhoSemantico(message, null);
if (conselho && conselho.index !== melhor.index && conselho.score >= CONFIANCA_ALTA) {
const info = { text: this.blocks[conselho.index], score: conselho.score, index: conselho.index };
return {
decision: 'RESPONDER', confidence: conselho.score, margem: 1, texto: info.text,
unificado: false, reanalisado: false, melhor: info, segundo: melhorInfo
};
}
return {
...
// nem BM25 focado, nem duplicado, nem fundível: última tentativa —
// pergunta ao conselheiro semântico pra desempatar só entre estes
// dois candidatos específicos (não o ficheiro todo, o universo já
// está reduzido a 2 blocos concorrentes).
const candidatosDesempate = segundo.index >= 0 ? [melhor.index, segundo.index] : [melhor.index];
const conselhoDesempate = this._conselhoSemantico(message, candidatosDesempate);
if (conselhoDesempate) {
const info = { text: this.blocks[conselhoDesempate.index], score: conselhoDesempate.score, index: conselhoDesempate.index };
return {
decision: 'RESPONDER', confidence: conselhoDesempate.score, margem, texto: info.text,
unificado: false, reanalisado: false, melhor: info, segundo: segundoInfo
};
}
...
module.exports = { KnowledgeIndex, KnowledgeCache, tokenizar, normalizar, stem, distanciaEdicao, diagnosticoTernlight };